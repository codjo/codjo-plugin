#!/bin/sh
#
# Script de demarrage/arret d'un serveur web ${artifactId}
#
...
LOG_DIR=${logDir}
MAIN_CLASS=${webMainClass}
tmp_dir=/tmp/${SERVER_NAME}

LOG_FILE=${LOG_DIR}/web-java.log

DEFAULT_ARG="-configuration ./web-config.properties"

PIDS=`/usr/ucb/ps auxwww | grep ${SERVER_NAME} | grep -v grep | gawk '{print $2}'`

# Pour demarrer un process
callJava() {
        java ${JAVA_WEB_OPTS} ${jvmArguments} -server -Djava.io.tmpdir=${tmp_dir}/ -Dfile.encoding=windows-1252 -Dlog.dir=$LOG_DIR -Dlog.filename=web.log -cp $MY_CLASSPATH $1 $2 >> ${LOG_FILE} 2>&1 &
        SERVER_PID=$!
        sleep 10
...
case "$1" in
  ...
  'status')
        if [ -z "${PIDS}" ]
        then
                echo "Status web server : OFF"
                exit 0
        else
                echo "Status web server : ON"
                exit 1
        fi
        ;;
*)
